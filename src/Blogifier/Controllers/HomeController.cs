...
namespace Blogifier.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(IBlogProvider blogProvider,
            IPostProvider postProvider, IFeedProvider feedProvider,
            IAuthorProvider authorProvider, IThemeProvider themeProvider,
        ...
        public async Task<IActionResult> Index(int page = 1)
        {
            ...
            return View($"~/Views/Themes/{model.Blog.Theme}/Index.cshtml",
                        model);
        }
        ...
        public async Task<IActionResult> Search(string term, int page = 1)
        {
            ...
            if (!string.IsNullOrEmpty(term))
            {
                ...
                string viewPath =
                    $"~/Views/Themes/{model.Blog.Theme}/Search.cshtml";
                                                    ...
                                                    int page = 1)
        {
            ...
            string viewPath =
                $"~/Views/Themes/{model.Blog.Theme}/Category.cshtml";
        ...
        public async Task<IActionResult> Error()
        {
            try
            {
                ...
                string viewPath =
                    $"~/Views/Themes/{model.Blog.Theme}/404.cshtml";
                if (IsViewExists(viewPath))
                    return View(viewPath, model);
        ...
        public async Task<IActionResult> Rss(string type)
        {
            ...
            using (var stream = new MemoryStream())
            {
                ...
                return File(
                    stream.ToArray(), "application/xml; charset=utf-8");
            }
        }
            ...
            try
            {
                ...
                if (!model.Post.Author.Avatar.StartsWith("data:"))
                    model.Post.Author.Avatar =
                        Url.Content($"~/{model.Post.Author.Avatar}");

                if (model.Post.PostType == PostType.Page)
                {
                    string viewPath =
                        $"~/Views/Themes/{model.Blog.Theme}/Page.cshtml";
                    if (IsViewExists(viewPath))
                        return View(viewPath, model);
                }

                return View($"~/Views/Themes/{model.Blog.Theme}/Post.cshtml",
                            model);
            }
            catch
            {
        ...
        public async Task<ListModel> getBlogPosts(string term = "",
            int pager = 1, string category = "", string slug = "")
        {
            ...
            if (!string.IsNullOrEmpty(category))
            {
                ...
                model.Posts = await _postProvider.GetList(
                    model.Pager, 0, category, "PF");
            }
            ...
            else
            {
                ...
                model.Posts = await _postProvider.Search(
                    model.Pager, term, 0, "FP");
            }

            if (model.Pager.ShowOlder) model.Pager.LinkToOlder =
                    $"?page={model.Pager.Older}";
            if (model.Pager.ShowNewer) model.Pager.LinkToNewer =
                    $"?page={model.Pager.Newer}";
